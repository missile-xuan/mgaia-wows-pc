...
const commonUtils = {
  ...
  // 字典项翻译
  getDictionariesLabel: function (dic, value) {
    if(commonUtils.isNull(value)){
      return "";
    }
    let dictionariesData = dictionaries.get(dic)
    let getLabel = (element) => {
      return element.value == value
    }
    return commonUtils.isNull(dictionariesData.find(getLabel))?value:dictionariesData.find(getLabel).label
  },
  getDictionaries: function (dic) {
    return dictionaries.get(dic)
  },
  //深拷贝对象
  ...
  formatNum(num,dp){
    ...
    if(num<0){result = '-'+numStr }
    if(this.isNull(dp)){return result}
    // 处理小数
